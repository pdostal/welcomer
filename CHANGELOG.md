...
- Config now uses named `[[message]]` templates and per-calendar `message` references, with
  `[[calendar]]` replacing `[[calendars]]`.
- The table adds a leading `#` column that counts only reservations outside today and in-progress
  stays.
